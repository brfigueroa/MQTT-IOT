...
window.onload = function(){
		...
		function onMessageArrived(mq) {
			...
			console.log(mq.destinationName + ' ' + mq.payloadString);
			//var json = JSON.parse(mq.payloadString );
	...
	function Connect() {
		...
		if (connected == false) {
			url = $("#url").val();
			client = new Paho.MQTT.Client(url,"myclientid_"+parseInt(Math.random() * 100, 10));
...
<body>
	<table width="850px" border="1">
		...
		<tr>
			...
			<td><input name="" class="button" id="url" value="ws://localhost:8000/" size="64"/></td>
		<tr>
			<td class="buttonx" onclick=Subscribe();>Subscribe</td>
			<td colspan="2"> <input name="" class="button" id="subscribe" value="utpl/#" size="64" /></td>
		</tr>
		<tr>
...
<td width="85%" class="header"><div id="header" onclick=ClearMsg();>Received Messages</div></td>
	...
	<button onclick=onclick=Connect()>Conectar Websockets</button>
	<button onclick=Subscribe()>Subscribirse Topico</button>
	<button onclick=Unsubscribe()>Desuscribirse Topico</button>
	<button onclick=Publish()>Publicar Topico</button>
